...
from pillboy.models import Singleton
...
class GUIConstants:
    ...
    ICON_FONT_NAME__PILLBOY = "seedsigner-glyphs"
    ICON_FONT_SIZE = 22
...
class Fonts(Singleton):
    ...
    def get_font(cls, font_name, size, file_extension: str = "ttf") -> ImageFont.FreeTypeFont:
        ...
        if font_name in [GUIConstants.ICON_FONT_NAME__FONT_AWESOME, GUIConstants.ICON_FONT_NAME__PILLBOY]:
            file_extension = "otf"
...
class Icon(BaseComponent):
    ...
    def __post_init__(self):
        ...
        if SeedSignerCustomIconConstants.MIN_VALUE <= self.icon_name and self.icon_name <= SeedSignerCustomIconConstants.MAX_VALUE:
            self.icon_font = Fonts.get_font(GUIConstants.ICON_FONT_NAME__PILLBOY, self.icon_size, file_extension="otf")
        else:
...
class Button(BaseComponent):
    # TODO: Rename the pillboy.helpers.Buttons class (to Inputs?) to reduce confusion
